...
/// @brief Constructor untuk membuat koneksi ke IoT Broker.
/// @param broker
/// @param port
/// @param clientId
...
UrusanIoT::UrusanIoT(const char* broker, const int port, const char* clientId,
    ...
    : _broker(broker), _port(port), _clientId(clientId), 
      _brokerUsername(brokerUsername), _brokerPassword(brokerPassword), client(4096) { }
...
/// @brief Cek apakah IoT sudah terkoneksi.
